...
class MusanNoise:
    ...
    SNR = {'noise':(0, 20), 'speech':(0, 20), 'music':(0, 20)}
    NumFile = {'noise': (1, 1), 'speech': (3, 7), 'music': (1, 1)}
    ...
    def __call__(self, x, category=None):
        # calculate dB
        x = x.np() if type(x) == torch.Tensor else x
        x_size = x.shape[-1]
        ...
        if len(noises) != 0:
            ...
            additive_noise = np.sqrt(10 ** (p / 10)) * noise
